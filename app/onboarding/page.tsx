...
export default function OnboardingPage() {
  ...
  /* ── Shared styles ── */
  const wrap: React.CSSProperties = { minHeight: '100vh', background: T.cream, color: T.slate, fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" };
  const box: React.CSSProperties = { maxWidth: 480, margin: '0 auto', padding: '2rem 1.5rem' };
  const logo: React.CSSProperties = { color: T.orange, fontSize: '1.5rem', fontWeight: 700, letterSpacing: '-0.02em', margin: 0 };
  const sub: React.CSSProperties = { color: T.muted, fontSize: '0.92rem', margin: '0.15rem 0 2rem' };
  const btn: React.CSSProperties = { background: T.slate, color: T.cream, border: 'none', padding: '0.9rem', borderRadius: 10, fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer', width: '100%', fontFamily: 'inherit' };
  ...
  const ta: React.CSSProperties = { width: '100%', background: T.cream, color: T.slate, border: `1px solid ${T.border}`, borderRadius: 8, padding: '0.7rem', fontSize: '0.85rem', lineHeight: '1.6', resize: 'vertical', fontFamily: 'inherit' };
