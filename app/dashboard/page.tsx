...
// Exact mockup colours
const V = {
  bg: '#F0EDE8', card: '#FAFAF8', card2: '#F5F3EF',
  orange: '#E8541A', orangeLight: '#FFF0EB',
  green: '#2D7A4F', greenLight: '#E8F5EE',
  amber: '#B8860B', amberLight: '#FFF8E6',
  text: '#1A1A1A', textMid: '#555', textSoft: '#999',
...
export default function DashboardPage() {
  ...
  if (loading) return <div style={{ display: 'flex', justifyContent: 'center', padding: '5rem 0' }}><div style={{ width: 24, height: 24, border: `3px solid ${V.card2}`, borderTopColor: V.orange, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /></div>;
  if (!d) return <div style={{ textAlign: 'center', padding: '5rem 1rem', fontFamily: sans }}><p style={{ color: V.textSoft }}>Could not load dashboard</p><button onClick={() => window.location.reload()} style={{ color: V.orange, background: 'none', border: 'none', cursor: 'pointer', fontFamily: sans, fontWeight: 500, marginTop: 8 }}>Try again</button></div>;
  ...
  const avgJob = 180;
  const estValue = estCalls * avgJob;
  const roi = estValue > 0 ? Math.round(estValue / 29) : 0;
  const totalActions = (p?.total_posts || 0) + (p?.total_replies || 0);

  // Card + label base styles matching mockup exactly
  const card: React.CSSProperties = { background: V.card, borderRadius: 16, padding: 16, boxShadow: V.shadow };
  const cardLabel: React.CSSProperties = { fontFamily: sans, fontSize: 10, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: V.textSoft, marginBottom: 10 };
  const badge = (bg: string, color: string): React.CSSProperties => ({ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 20, background: bg, color, marginTop: 6 });

  return (
    <>
      <style>{`@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.6;transform:scale(.85)}} @keyframes fadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}`}</style>

      {/* Business header */}
      <div style={{ padding: '4px 0 8px', animation: 'fadeUp .5s ease both' }}>
        <div style={{ fontSize: 20, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, fontFamily: sans }}>
          {p?.business_name || d.user.name}
          <span style={{ width: 8, height: 8, background: V.green, borderRadius: '50%', animation: 'pulse 2s infinite', flexShrink: 0 }} />
        </div>
      ...
      {/* Score + Managed */}
      {hasScore && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, animation: 'fadeUp .5s .05s ease both' }}>
          <div style={{ ...card, padding: '14px 16px' }}>
            <div style={cardLabel}>Profile Score</div>
            <div style={{ fontSize: 38, fontWeight: 600, lineHeight: 1, marginTop: 6, fontFamily: mono, color: sc(p.audit_score_after || p.audit_score) }}>
              {p.audit_score_after || p.audit_score}<span style={{ fontSize: 16, color: V.textSoft }}>/100</span>
            </div>
            {p.audit_score_after && p.audit_score && <div style={badge(V.greenLight, V.green)}>↑ +{p.audit_score_after - p.audit_score} from setup</div>}
            {/* Sparkline */}
            <div style={{ marginTop: 8 }}>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 28 }}>
                {[20,30,35,45,55,65,72, (p.audit_score_after || p.audit_score)].map((h, i) => (
                  <div key={i} style={{ flex: 1, height: `${h}%`, borderRadius: '3px 3px 0 0', background: i === 7 ? V.orange : V.orangeLight, transition: 'height .3s' }} />
                ))}
              </div>
            </div>
          </div>
          <div style={{ ...card, padding: '14px 16px' }}>
            <div style={cardLabel}>Managed For</div>
            <div style={{ fontSize: 38, fontWeight: 600, lineHeight: 1, marginTop: 6, fontFamily: mono, color: V.text }}>
              {p.streak_weeks || 0}<span style={{ fontSize: 14, color: V.textSoft }}> wks</span>
            </div>
            <div style={badge(V.greenLight, V.green)}>0 tasks missed</div>
            <div style={{ marginTop: 10 }}>
              <div style={{ fontSize: 11, color: V.textSoft }}>Actions taken</div>
              <div style={{ fontSize: 18, fontWeight: 600, fontFamily: mono, marginTop: 2 }}>{totalActions}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Value banner */}
      {estCalls > 0 && (
        <div style={{ background: V.text, color: 'white', borderRadius: 16, padding: '16px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', animation: 'fadeUp .5s .1s ease both' }}>
          <div>
            <div style={{ fontSize: 11, opacity: 0.5, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4, fontFamily: sans }}>Est. work generated</div>
            <div style={{ fontSize: 28, fontWeight: 600, fontFamily: mono, color: '#fff' }}>£{estValue.toLocaleString()}</div>
            <div style={{ fontSize: 11, opacity: 0.4, marginTop: 2, fontFamily: sans }}>Based on {estCalls} calls × £{avgJob} avg job</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 22, fontWeight: 600, fontFamily: mono, color: '#7DFF9B' }}>{roi}×</div>
            <div style={{ fontSize: 10, opacity: 0.4, textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: sans }}>return on £29/mo</div>
          </div>
        </div>
      )}
      ...
      {g?.metrics && (g.metrics.views + g.metrics.calls + g.metrics.directions + g.metrics.websiteClicks > 0) && (
        <div style={{ ...card, animation: 'fadeUp .5s .15s ease both' }}>
          <div style={cardLabel}>Google Stats · Last 28 days</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            <SC num={g.metrics.views.toLocaleString()} label="Profile views" />
            <SC num={String(g.metrics.calls)} label="Calls from profile" highlight />
            <SC num={String(g.metrics.directions)} label="Directions" />
            <SC num={String(g.metrics.websiteClicks)} label="Website clicks" />
            <SC num={String(g.metrics.totalActions)} label="Total actions" highlight />
          ...
          <div style={{ background: V.greenLight, borderRadius: 12, padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
            ...
            <span style={{ fontSize: 16, fontWeight: 600, fontFamily: mono, color: V.green }}>6.5 hrs</span>
          </div>
        </div>
      )}
      ...
      {g?.reviews && g.reviews.total > 0 && (
        <div style={{ ...card, animation: 'fadeUp .5s .25s ease both' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <span style={cardLabel}>Reviews</span>
      ...
      {g?.profile && (
        <div style={{ ...card, animation: 'fadeUp .5s .3s ease both' }}>
          <div style={cardLabel}>Profile Health</div>
      ...
      {d.activity?.length > 0 && (
        <div style={{ ...card, animation: 'fadeUp .5s .35s ease both' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <span style={{ ...cardLabel, marginBottom: 0 }}>Recent Activity</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {d.activity.slice(0, 4).map((a: any, i: number) => (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0, marginTop: 1, background: a.type === 'post' ? V.orangeLight : a.type === 'reply' ? V.greenLight : '#EEF2FF' }}>
                  {a.type === 'post' ? '📝' : a.type === 'reply' ? '⭐' : '📊'}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 500, lineHeight: 1.3, fontFamily: sans }}>{a.action}</div>
                  <div style={{ fontSize: 11, color: V.textSoft, marginTop: 2, fontFamily: sans }}>{timeAgo(a.date)}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Posts + Replies */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, animation: 'fadeUp .5s .4s ease both' }}>
        <div style={{ ...card, padding: '14px 16px' }}>
          <div style={cardLabel}>Posts by us</div>
          <div style={{ fontSize: 38, fontWeight: 600, lineHeight: 1, fontFamily: mono, color: V.text }}>{p?.total_posts || 0}</div>
          <div style={{ fontSize: 12, color: V.textSoft, marginTop: 1, fontFamily: sans }}>since you joined</div>
        </div>
        <div style={{ ...card, padding: '14px 16px' }}>
          <div style={cardLabel}>Replies by us</div>
          <div style={{ fontSize: 38, fontWeight: 600, lineHeight: 1, fontFamily: mono, color: V.text }}>{p?.total_replies || 0}</div>
          <div style={{ fontSize: 12, color: V.textSoft, marginTop: 1, fontFamily: sans }}>reviews handled</div>
        </div>
      </div>
      ...
      {!g && d.recentPosts?.length === 0 && (
        <div style={{ ...card, textAlign: 'center', padding: '2rem' }}>
          <p style={{ fontWeight: 600, fontSize: 15, margin: '0 0 4px', fontFamily: sans }}>Your first post is on its way</p>
          <p style={{ fontSize: 13, color: V.textSoft, margin: 0, fontFamily: sans }}>We will write and publish to your Google profile this week.</p>
        </div>
      )}

      {/* Refer a mate */}
      <div style={{ ...card, animation: 'fadeUp .5s .45s ease both' }}>
...
function SC({ num, label, highlight, star }: { num: string; label: string; highlight?: boolean; star?: boolean }) {
  return (
    <div style={{ background: '#F5F3EF', borderRadius: 12, padding: '12px 10px', textAlign: 'center' }}>
      <div style={{ fontSize: 22, fontWeight: 600, fontFamily: "'DM Mono', monospace", lineHeight: 1, color: highlight ? '#E8541A' : '#1A1A1A' }}>
        {num}{star && <span style={{ color: '#FBBC04', fontSize: 14, marginLeft: 2 }}>{'\u2605'}</span>}
      </div>
...
function HI({ ok, label, val }: { ok: boolean; label: string; val: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>
      <span style={{ width: 7, height: 7, borderRadius: '50%', background: ok ? '#2D7A4F' : '#E8A020', flexShrink: 0 }} />
      {label}
      <span style={{ marginLeft: 'auto', fontSize: 11, color: '#999' }}>{val}</span>
    </div>
  );
}
...
function CopyBtn({ code }: { code: string }) {
  ...
  return (
    <button onClick={() => { navigator.clipboard.writeText(`https://chocka.co.uk/ref/${code}`); setCopied(true); setTimeout(() => setCopied(false), 2000); }} style={{ background: '#1A1A1A', color: 'white', border: 'none', borderRadius: 10, padding: '9px 16px', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
      {copied ? 'Copied!' : 'Copy'}
    </button>
  );
}
